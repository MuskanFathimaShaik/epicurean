...
const LikedRecipesPage = () => {
  ...
  const [unlikingRecipe, setUnlikingRecipe] = useState(null);
  ...
  useEffect(() => {
    const loadLikedRecipes = async () => {
      try {
        setIsLoading(true);
        const likedRecipesData = localStorage.getItem("likedRecipes");
        if (likedRecipesData) {
          const likedRecipes = JSON.parse(likedRecipesData);
          await fetchRecipeDetails(likedRecipes);
        }
      } catch (error) {
  ...
  const fetchRecipeDetails = async (likedRecipesList) => {
    try {
      const detailedRecipes = await Promise.all(
        likedRecipesList.map(async (recipeName) => {
          try {
  ...
  // Professional unlike functionality
  const handleUnlike = async (recipeName) => {
    setUnlikingRecipe(recipeName);
    ...
    setUnlikingRecipe(null);

    // Update localStorage
    const likedRecipeNames = updatedRecipes.map((r) => r.strMeal);
    localStorage.setItem("likedRecipes", JSON.stringify(likedRecipeNames));
  ...
  // Quick unlike with confirmation
  const handleQuickUnlike = (recipeName, e) => {
    e.stopPropagation();
    e.preventDefault();

    if (window.confirm(`Remove "${recipeName}" from your liked recipes?`)) {
      handleUnlike(recipeName);
    }
  };
          ...
          <div className="bg-white rounded-xl flex items-center justify-between shadow-lg border border-gray-200 p-3 md:p-4">
            <div className="flex items-center gap-3 flex-1">
              <div className="bg-red-100 p-2 rounded-lg flex-shrink-0">
                <Trash2 className="text-red-600 w-4 h-4 md:w-5 md:h-5" />
              </div>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500 truncate">
                  {removedRecipe?.strMeal} was removed from likes
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0 ml-2">
              <button
                ...
                className="flex items-center space-x-1 bg-red-600 text-white px-2 py-1.5 md:px-3 md:py-1.5 rounded-lg text-xs md:text-sm font-medium hover:bg-red-700 transition-colors"
              >
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            ...
            <div className="flex items-center justify-between w-full sm:w-auto">
              <div className="flex items-center space-x-3 md:space-x-4">
                {/* Compact Icon */}
                <div className="bg-gradient-to-br from-red-500 to-pink-600 p-2.5 md:p-3 rounded-lg md:rounded-2xl shadow-sm">
                  <Heart
              ...
              <span className="md:hidden font-bold text-pink-600">
                {recipes.length}
              </span>
            </div>

            {/* Desktop Stats */}
            <div className="hidden sm:block bg-red-50 rounded-xl px-4 py-3 border border-red-200">
              <div className="text-center">
            ...
            <div className="flex flex-col lg:flex-row gap-3 md:gap-4 items-center justify-between">
              ...
              <div className="relative w-full lg:flex-1 lg:max-w-lg">
                ...
                <input
                  ...
                  className="w-full pl-10 sm:pl-12 pr-8 sm:pr-10 py-2.5 sm:py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all duration-300 text-sm sm:text-base"
                />
                {searchTerm && (
                  <button
                    onClick={clearSearch}
              ...
              <div className="flex items-center justify-between w-full lg:w-auto gap-3 md:gap-4">
                ...
                <div className="flex items-center space-x-2 md:space-x-3">
                  ...
                  <select
                    ...
                    className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 md:px-4 md:py-2.5 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all duration-300 text-sm md:text-base w-32 md:w-auto"
                  >
                ...
                <div className="flex bg-gray-50 border border-gray-200 rounded-lg overflow-hidden">
                  <button
                    ...
                    className={`p-2 transition-all duration-300 ${
                      viewMode === "grid"
                        ? "bg-gradient-to-br from-red-500 to-pink-600 text-white shadow-sm"
        ...
        {recipes.length === 0 ? (
          <div className="text-center py-12 md:py-16">
            ...
            <button
              ...
              className="bg-gradient-to-br from-red-500 to-pink-600 text-white px-6 py-3 md:px-8 md:py-3.5 rounded-xl font-semibold hover:bg-red-700 transition-all duration-300 shadow-sm hover:shadow-md text-sm md:text-base"
            >
              Discover Recipes
            </button>
          </div>
          ...
          <div className="text-center py-12 md:py-16">
            ...
            <button
              onClick={clearSearch}
              className="bg-gradient-to-br from-red-500 to-pink-600 text-white px-6 py-3 md:px-8 md:py-3.5 rounded-xl font-semibold hover:bg-red-700 transition-all duration-300 text-sm md:text-base"
            >
              Clear Search
            </button>
          </div>
          ...
          >
            {filteredAndSortedRecipes.map((recipe) => {
              ...
              const isUnliking = unlikingRecipe === recipe.strMeal;
              ...
              return (
                <div
                  ...
                  className={`bg-white rounded-xl md:rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300 ${
                    ...
                    isUnliking ? "opacity-50 scale-95" : "opacity-100 scale-100"
                  }`}
                >
                  ...
                  >
                    ...
                    />
                    <div className="absolute top-3 left-3 bg-gradient-to-br from-red-500 to-pink-600 text-white px-2 py-1 md:px-3 md:py-1.5 rounded-lg text-xs md:text-sm font-semibold shadow-sm">
                      {recipe.strArea}
                    </div>
                    <button
                      onClick={(e) => handleQuickUnlike(recipe.strMeal, e)}
                      disabled={isUnliking}
                      className={`absolute top-3 right-3 p-2 rounded-lg md:rounded-xl transition-all duration-300 shadow-sm hover:shadow-md ${
                        isUnliking
                          ...
                          : "bg-white/90 hover:bg-white text-red-600 hover:scale-110"
                      }`}
                      title="Remove from liked recipes"
                    >
                      {isUnliking ? (
                  ...
                  <div className="flex-1 p-4 md:p-6 flex flex-col">
                    ...
                    <div className="mb-4 md:mb-6 grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4">
                      <div className="flex items-center space-x-1 md:space-x-2 text-xs md:text-sm text-gray-600">
                        <Clock size={14} className="text-red-600 md:w-4" />
                      ...
                      <div className="flex items-center space-x-1 md:space-x-2 text-xs md:text-sm text-gray-600">
                        <Star
                          size={14}
                          className="text-blue-600 fill-current md:w-4"
                        />
                        <span>{recipe.rating}</span>
                      </div>
                      <div className="flex items-center space-x-1 md:space-x-2 text-xs md:text-sm text-gray-600">
                        <Flame size={14} className="text-purple-600 md:w-4" />
                    ...
                    <div className="mb-4 md:mb-6">
                      ...
                      <div className="flex flex-wrap gap-1.5 md:gap-2">
                        {ingredients.slice(0, 3).map((item, index) => (
                          <span
                            key={index}
                            className="bg-red-50 text-red-700 px-2 py-1 md:px-3 md:py-1.5 rounded-lg text-xs md:text-sm border border-red-200 font-medium"
                          >
                            {item.ingredient}
                          </span>
                        ))}
                    ...
                    <div className="mt-auto flex flex-col sm:flex-row gap-2 md:gap-3">
                      <button
                        ...
                        className="flex-1 bg-gradient-to-br from-red-500 to-pink-600 text-white py-2.5 md:py-3 px-4 rounded-xl font-semibold hover:bg-red-700 transition-all duration-300 flex items-center justify-center space-x-2 shadow-sm hover:shadow-md text-sm md:text-base"
                      >
